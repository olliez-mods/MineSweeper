...
std::vector<std::vector<Space>> createBoard(int board_size, sf::RenderWindow& window)
{
	...
	for (int x = 0; x < board_size; x++)
	{
		...
		for (int y = 0; y < board_size; y++)
		{
			...
			new_rect.setPosition(x * (window.getSize().x / board_size), y * (window.getSize().y / board_size));
...
void emptyBoard(std::vector<std::vector<Space>> &board, sf::RenderWindow& window)
{
	for (int x = 0; x < board.size(); x++)
	{
		for (int y = 0; y < board.size(); y++)
		{
			...
			new_rect.setPosition(x * (window.getSize().x / board.size()), y * (window.getSize().y / board.size()));
			board[x][y].setRect(new_rect);
			board[x][y].texture.loadFromFile("sprites/hidden.png");
